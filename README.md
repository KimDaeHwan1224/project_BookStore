...
- 🗂 **DB 설계** — 유저 / 탈퇴 회원 관리 테이블 구조 설계  
- 🔐 **인증/인가** — 회원가입·로그인/로그아웃, 소셜 로그인(Naver/Google), 아이디·비밀번호 찾기, 이메일 인증, 세션 기반 권한 처리  
- 👤 **사용자 기능** — 메인페이지·마이페이지, 상품 리스트·도서 검색, 찜 목록, 1:1 문의, 공지사항 조회 등 주요 화면 및 기능 구현  
- 🛠 **관리자 기능** — 관리자 모드 프론트엔드(대시보드, 회원/탈퇴 회원 관리, 게시판·공지사항 관리, 주문 리스트·주문 상세, 문의 리스트) UI 및 백엔드 연동  
- 💬 **커뮤니티 기능** — 사용자 게시판 및 공지사항 조회, 조회수 증가·페이징 처리 등 사용자 커뮤니티 기능 구현  
